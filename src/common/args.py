...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        dest="dirtydb",
        default=False,
        help="allow dirty DB with uncommited changes",
    )

    run_parser = subparsers.add_parser(
        "generate", help="run system call usage analysis for an application"
    )

    run_parse_req_args = run_parser.add_argument_group("required arguments")
    run_parse_req_args.add_argument(
        "-db",
        "--database",
        dest="dbpath",
        type=pathlib.Path,
    ...
    run_parse_req_args.add_argument(
        ...
        type=str,
        required=True,
        help="name of the application to be analyzed (e.g., nginx)",
        dest="application",
    )
    run_parse_req_args.add_argument(
        "-w",
        ...
        type=str,
        required=True,
        help="name of the workload (e.g., wrk)",
        dest="workload",
    )
    run_parse_req_args.add_argument(
        "-d",
    ...
    run_parse_classifier_args = run_parser.add_argument_group(
        "classifier arguments (exactly one required)"
    )
    run_parse_classifier_args.add_argument(
        "-b",
        action="store_true",
        dest="isbenchmark",
    ...
    run_parse_other_args.add_argument(
        ONLY_DOCKER_OPT,
        action="store_true",
        dest="onlydocker",
    ...
    search_parser = subparsers.add_parser(
        "search", help="retrieve and analyze data from the database"
    )
    ...
    action_args.add_argument(
        ...
        action="store_true",
        help="output the histogram of the paper, ignores passed set",
        dest="paperhistogramplot",
    )
    action_args.add_argument(
        "--export-sqlite", action="store_true", help="export the DB as SQLite database"
    )
    ...
    opt_args.add_argument(
        "--static-source",
        action="store_true",
        help="also include static source analysis data",
        dest="ssource",
    )
    opt_args.add_argument(
        "--output-sys-names",
